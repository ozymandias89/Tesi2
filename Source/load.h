...

	{
		...
		for (std::vector<double*>::const_iterator j = cut_A.begin();
			...
			for (int iter = 0; iter < N; iter++) {
				...
				if (ptr[iter] != 0) {
					idx.push_back(num_rows + 1 + iter);
					coef.push_back(ptr[iter]);
					nzcnt++;
				}

			}

			r += cut_b[iterator_b] * min_sol
					+ dual_varVals_P1[num_constraint + iterator_b]
					+ dual_varVals_P2[num_constraint + iterator_b];

			//b[i]*beta
			if (cut_b[iterator_b] != 0) {
				idx.push_back(num_rows + 1 + N);
				coef.push_back(cut_b[iterator_b]);
				nzcnt++;
			}

			//u_i
			idx.push_back(num_constraint + 1 + iterator_b);
			coef.push_back(1);
			nzcnt++;

			//v_i
			idx.push_back(num_rows + 3 + N + num_constraint + iterator_b);
			coef.push_back(1);
			nzcnt++;
